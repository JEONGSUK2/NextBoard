...
useEffect(()=>{
    ...
    const fetchData = async () => {
        ...
        setPost(data.data)  //setPost라는 변수안에 data.data의 데이터를 넣는다.
        setIsLoading(false)
    }   
    fetchData();
},[params.id])
        ...
        {isLoading && <Lodaing/>}
            {
                post.length > 0 && (
                    ...
                    <p className="basis-1/6">작성자 :{post[0]?.title}</p>
                    <p className="basis-1/6">정보 :{post[0]?.userid}</p>
                    <p className="basis-1/6">세션 :{session&&session.user.email === post[0]?.userid ? '같음' : '다름'}</p>
                    <p className="basis-1/6">타이틀:{post[0]?.username}</p>
                    </div>
                    {
            ...
            }
            {
              ...
              <>  
               ...
               <Link href="/edit/id"><button className="bg-orange-500 text-white px-4 py-2 rounded shadow-md hover:bg-orange-600 focus:outline-none mr-1">수정</button></Link>
                <button className="bg-green-500 text-white px-4 py-2 rounded shadow-md hover:bg-green-600 focus:outline-none" onClick={()=>{deletePost(post&&post[0]?.id)}}>삭제</button>
              </>
            }
        </>
    )
